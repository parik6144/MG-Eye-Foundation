    ...
    <div className="pt-28 pb-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {EQUIPMENT.map((eq, i) => (
            <article key={eq.name} className="grid lg:grid-cols-2 gap-8 lg:gap-14 items-center">
              ...
              <div className={i % 2 === 1 ? "lg:order-1" : undefined}>
                <span className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-[#1118E8] text-white font-bold">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <p className="mt-5 text-[#FF741F] text-xs font-semibold tracking-[0.18em] uppercase">{eq.use}</p>
                <h2 className="mt-2 text-3xl sm:text-4xl font-extrabold text-[#0F172A] leading-tight">{eq.name}</h2>
                <div className="mt-8 space-y-5">
                  <p className="text-[#475569] text-lg leading-relaxed">
                    <span className="block text-[#1118E8] text-sm font-semibold mb-1">What it is</span>
                    {eq.what}
                  </p>
                  <p className="text-[#475569] text-lg leading-relaxed">
                    <span className="block text-[#FF741F] text-sm font-semibold mb-1">What you feel</span>
                    {eq.feel}
                  </p>
                  <p className="text-[#475569] text-lg leading-relaxed">
        ...
        <div className="mt-20 rounded-[28px] bg-[#F7F9FC] border border-[#e2e8f0] px-8 py-10 text-center">
          ...
          <a
            href={SITE.phoneHref}
            className="mt-6 inline-flex bg-[#1118E8] hover:bg-[#0d12b8] text-white font-bold rounded-full px-8 py-3 text-sm"
          >
