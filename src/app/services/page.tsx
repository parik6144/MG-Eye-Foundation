    ...
    <div className="pt-6 pb-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-14 text-center">
          <span className="text-[#1F8A9A] text-sm font-semibold tracking-wide uppercase">Services</span>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {SERVICES.map((s) => (
            <article key={s.title} className="rounded-[20px] overflow-hidden border border-[#e2e8f0] bg-white hover:shadow-lg hover:shadow-[#1F8A9A]/[0.06] transition-all">
              <img src={s.image} alt={s.title} className="h-48 w-full object-cover" />
              <div className="p-6">
                <h2 className="text-lg font-extrabold text-[#0F172A]">{s.title}</h2>
                <p className="text-sm text-[#475569] mt-3 leading-relaxed"><span className="font-semibold text-[#1F8A9A]">Condition: </span>{s.what}</p>
                <p className="text-sm text-[#475569] mt-2 leading-relaxed"><span className="font-semibold text-[#1F8A9A]">Common Symptoms: </span>{s.feel}</p>
                ...
                <ul className="mt-2 space-y-1.5">
                  {s.offers.map((item) => (
                    <li key={item} className="text-sm text-[#475569] leading-snug pl-3 border-l-2 border-[#1F8A9A]/30">
                      {item}
                    </li>
                  ))}
                </ul>
                <div className="mt-5 flex flex-wrap items-center gap-3">
                  <button onClick={book} className="inline-flex bg-[#1F8A9A] hover:bg-[#16707E] text-white font-bold rounded-full px-5 py-2.5 text-sm">
                    Book this care
                  </button>
                  <Link href={`/services/${s.slug}`} className="text-sm font-bold text-[#1F8A9A] hover:underline">
                    View more
                  </Link>
                </div>
              </div>
